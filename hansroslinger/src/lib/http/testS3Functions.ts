...
const testS3Functions = async () => {
  ...
  try {
    ...
    userFiles.forEach((file) => {
      console.log(
        `${file.key} (${file.size} bytes) - Last modified: ${file.lastModified}`,
      );
    });

    if (userFiles.length === 0) {
      console.log(
        "No files found. Cannot test getObject, downloadFile, or deleteFile.",
      );
      return;
    }
    ...
    console.log(
      `   - Body: ${objectData.body ? "Stream available" : "No body"}`,
    );
    ...
    if (filesAfterDelete.length === 0) {
      ...
      console.log("File may still exist:");
