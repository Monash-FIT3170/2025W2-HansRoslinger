...
export async function getJson<TResponse>(url: string, init?: RequestInit): Promise<TResponse> {
...
export async function deleteJson<TResponse>(url: string, init?: RequestInit): Promise<TResponse> {
	const response = await fetch(composeUrl(url), {
		method: "DELETE",
		headers: {
			Accept: "application/json",
